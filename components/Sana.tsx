...
export default function Sana() {
  ...
  const sendMessageToApi = async (userMessage: string): Promise<string> => {
    ...
    const payload = {
      unique_id: uniqueId,
      query: userMessage,
      history: chatHistoryText,           // ← correct field name
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 422) {
          try {
            ...
            const msg = json.detail?.[0]?.msg || "Invalid request format";
            return `Validation error: ${msg}`;
          } catch {}
        }
        return `Server error (${res.status}): ${errText || "No details provided"}`;
      }
      ...
      return `Connection failed: ${msg}. Please check your internet or try again later.`;
    }
  };
